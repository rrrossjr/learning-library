...
Several initialization parameters control the memory usage of a PDB.
 ...
 DB\_CACHE\_SIZE, SHARED\_POOL\_SIZE, SGA\_MIN\_SIZE, SGA\_TARGET, PGA\_AGGREGATE\_LIMIT and PGA\_AGGREGATE\_TARGET can be set for a PDB, ensure that the CDB and the other PDBs have sufficient memory for their operations. The initialization parameters control the memory usage of PDBs only if the following conditions are met:
